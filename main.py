...
list_of_codes = ['USD', 'RUB']  # список валют в транзакциях
code = 'US'
if code not in list_of_codes:  # проверяем првильность выбора валюты
    print(f'Выберите валюту {", ".join(list_of_codes)}')
else:
    usd_transactions = filter_by_currency(transactions, code)  # запускаем генератор с отфильтрованными словарями
    for _ in range(4):
        try:
            print(next(usd_transactions))
        except StopIteration:
            print("Список транзакций закончился")  # выводим сообщение об окончании итераций
print('\n' + '=' * 10 + '\n')  # визуальный разделитель
...
for card_number in card_number_generator(1, 5):  # запускаем генератор рандомных номеров карт
    print(card_number)